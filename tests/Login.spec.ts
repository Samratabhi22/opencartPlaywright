...
test('User login test @master @sanity @regression',async({ page })=>{
    const registeredUser = await registerUserAndLogout(page, 'test123');
    ...
    await loginPage.setEmail(registeredUser.email);
    await loginPage.setPassword(registeredUser.password);
